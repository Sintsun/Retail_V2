...
def draw_axes(img, origin, imgpts):
    ...
    :param img: 要绘制的图像。
    :param origin: 原点的图像坐标 (tuple of ints)。
    :param imgpts: 投影后的轴点的图像坐标 (numpy array of shape (3, 2))。
    :return: 绘制坐标轴后的图像。
    """
...
def main():
    ...
    dist_coeffs_path = os.path.join(image_folder, "distortion_coeffs.npy")
    ...
    try:
        ...
        # 设置激光功率为最大值，并确保符合步长
        desired_laser_power = max_laser_power
            ...
            if sampling and vertices_global is not None:
                if sample_counter < SAMPLE_COUNT:
                    ...
                    for dy in range(-half_size, half_size + 1):
                        for dx in range(-half_size, half_size + 1):
                            ...
                            if 0 <= nx < img_width and 0 <= ny < img_height:
                                point_camera = vertices_global[ny, nx]
                                if np.all(np.isfinite(point_camera)) and not np.all(point_camera == 0):
                                    samples.append(point_camera)
                    if samples:
                        sampled_points.append(np.mean(samples, axis=0).reshape(3, 1))
                    ...
                    if sampled_points:
                        ...
                        print(
                            f"相机坐标系下的3D点（平均）: X={avg_point_camera[0][0]:.4f} m, "
                            f"Y={avg_point_camera[1][0]:.4f} m, Z={avg_point_camera[2][0]:.4f} m")
                        print(
                            f"世界坐标系下的3D点（平均）: X={point_world[0][0]:.4f} m, "
